...
class OtgruzkaWindow(QtWidgets.QWidget, Ui_OtgruzkaWindow.Ui_OtgruzkaWindow):
    ...
    def doPrn(self):
        ...
        if ok:
            ...
            setup.setOption(QtPrintSupport.QAbstractPrintDialog.PrintToFile, on=False)
            setup.setOption(QtPrintSupport.QAbstractPrintDialog.PrintPageRange, on=False)
            setup.setOption(QtPrintSupport.QAbstractPrintDialog.PrintCollateCopies, on=False)
            setup.setOption(QtPrintSupport.QAbstractPrintDialog.PrintCurrentPage, on=False)
            setup.exec()
            ...
            if self.size.currentText() != '':
                if self.source.currentText() != '':
                    if self.count.value() > 0:
                        ...
                        result = self.db.runSql("SELECT * FROM CODES WHERE \\"
                                                "GTIN = '%s' AND SOURCE = '%s' AND STATE = '0';" %
